...
var StakingApp = (() => {
  ...
  __export(index_exports, {
    ...
    PID: () => PID,
    STAKE_TOKEN_ABI: () => STAKE_TOKEN_ABI,
    ...
    getCurrentAPY: () => getCurrentAPY,
    getNetwork: () => getNetwork,
    getPendingReward: () => getPendingReward,
    getPendingWithdrawals: () => getPendingWithdrawals,
    getPoolInfo: () => getPoolInfo,
    getReadOnlyContract: () => getReadOnlyContract,
    getReadOnlyTokenContract: () => getReadOnlyTokenContract,
    getStakeInfo: () => getStakeInfo,
    getStakeInfoPublic: () => getStakeInfoPublic,
    getTokenBalance: () => getTokenBalance,
    getTokenContract: () => getTokenContract,
    getTotalDeposited: () => getTotalDeposited,
    getUserAddress: () => getUserAddress,
    getUserInvestInfo: () => getUserInvestInfo,
    getWithdrawalLockPeriod: () => getWithdrawalLockPeriod,
  ...
  // abi/MasterChefStakingABI.json
  var MasterChefStakingABI_default = [
    {
      inputs: [],
  ...
  // abi/StakeTokenABI.json
  var StakeTokenABI_default = [
    {
      inputs: [],
      name: "name",
  ...
  // src/web3/config.ts
  var CONTRACT_ADDRESS = "0x6B9BA977D5e65a68F3E06C253d0504a932138453";
  var STAKING_TOKEN_ADDRESS = "0x55d398326f99059fF775485246999027B3197955";
  var PID = 0;
  var NETWORK_CHAIN_ID = 56;
  var ABI = MasterChefStakingABI_default;
  var STAKE_TOKEN_ABI = StakeTokenABI_default;
  ...
  function buildWrappedFallback(contract2) {
    ...
    const send = async function(overrides) {
      ...
      return new ContractTransactionResponse(contract2.interface, provider2, tx);
    };
  ...
  var NonceManager = class _NonceManager extends AbstractSigner {
    ...
    constructor(signer2) {
      ...
      defineProperties(this, { signer: signer2 });
  ...
  var BSC_RPC_URL = "https://bsc-dataseed.binance.org/";
  var provider = null;
  var signer = null;
  var contract = null;
  var readOnlyContract = null;
  var tokenContract = null;
  var readOnlyTokenContract = null;
  var userAddress = null;
  var publicRpcProvider = null;
  var currentWalletType = null;
  ...
  function getReadOnlyContract() {
    ...
    if (!publicRpcProvider) {
      publicRpcProvider = new ethers_exports.JsonRpcProvider(BSC_RPC_URL);
    }
    readOnlyContract = new ethers_exports.Contract(CONTRACT_ADDRESS, ABI, publicRpcProvider);
  ...
  function getReadOnlyTokenContract() {
    ...
    readOnlyTokenContract = new ethers_exports.Contract(STAKING_TOKEN_ADDRESS, STAKE_TOKEN_ABI, publicRpcProvider);
  ...
  function getAvailableWallets() {
    return [
      ...
      // WalletConnect is always "available" via modal
    ];
  }
  async function connectWithEthereum(walletType) {
    ...
    try {
      provider = new ethers_exports.BrowserProvider(ethereumProvider);
      await provider.send("eth_requestAccounts", []);
      signer = await provider.getSigner();
      userAddress = await signer.getAddress();
      contract = new ethers_exports.Contract(CONTRACT_ADDRESS, ABI, signer);
      readOnlyContract = new ethers_exports.Contract(CONTRACT_ADDRESS, ABI, provider);
      tokenContract = new ethers_exports.Contract(STAKING_TOKEN_ADDRESS, STAKE_TOKEN_ABI, signer);
      readOnlyTokenContract = new ethers_exports.Contract(STAKING_TOKEN_ADDRESS, STAKE_TOKEN_ABI, provider);
  ...
  async function getCurrentAPY() {
    const contract2 = getReadOnlyContract();
    if (!contract2) throw new Error("Read-only contract not available");
    return await contract2.getCurrentAPY();
  }
  async function getTotalDeposited() {
    const contract2 = getReadOnlyContract();
    if (!contract2) throw new Error("Read-only contract not available");
    return await contract2.getTotalDeposited();
  }
  async function getPoolInfo(pid = PID) {
    const contract2 = getReadOnlyContract();
    if (!contract2) throw new Error("Read-only contract not available");
    const [stakeToken, totalDeposited, accRewardPerToken, lastUpdateTime] = await contract2.getPoolInfo(pid);
    return {
      stakeToken,
      totalDeposited,
      accRewardPerToken,
  ...
  async function getUserInvestInfo(pid = PID, address) {
    const contract2 = getContract();
    if (!contract2) throw new Error("Staking contract not available");
    const userAddress2 = address || getUserAddress();
    if (!userAddress2) throw new Error("No user address");
    return await contract2.getUserInvestInfo(pid, userAddress2);
  }
  async function getPendingReward(pid = PID, address) {
    const contract2 = getContract();
    if (!contract2) throw new Error("Staking contract not available");
    const userAddress2 = address || getUserAddress();
    if (!userAddress2) throw new Error("No user address");
    return await contract2.getPendingReward(pid, userAddress2);
  }
  async function getTokenBalance(address) {
    const contract2 = getReadOnlyTokenContract();
    if (!contract2) throw new Error("Token read-only contract not available");
    const userAddress2 = address || getUserAddress();
    if (!userAddress2) throw new Error("No user address");
    return await contract2.balanceOf(userAddress2);
  }
  async function getStakeInfo(pid = PID) {
  ...
  async function getStakeInfoPublic(pid = PID) {
    const [tvl, apy] = await Promise.all([
      getTotalDeposited(),
      getCurrentAPY()
    ]);
  ...
  async function getWithdrawalLockPeriod() {
    const contract2 = getReadOnlyContract();
    if (!contract2) throw new Error("Read-only contract not available");
    return await contract2.withdrawalLockPeriod();
  }
  async function getWithdrawalRequests(pid = PID) {
    const contract2 = getReadOnlyContract();
    if (!contract2) throw new Error("Read-only contract not available");
    const userAddress2 = getUserAddress();
    if (!userAddress2) throw new Error("No user address");
    const lockPeriod = await contract2.withdrawalLockPeriod();
    const result = await contract2.withdrawalRequests(userAddress2, pid);
  ...
  async function getPendingWithdrawals() {
    ...
    const userAddress2 = getUserAddress();
    if (!userAddress2) throw new Error("No user address");
    const [pids, amounts, requestTimes, claimableAts] = await contract2.getPendingWithdrawals();
    const withdrawals = [];
  ...
  async function deposit(amount, pid = PID) {
    if (!isWalletConnected()) {
      return { success: false, message: "Please connect your wallet" };
    }
    const contract2 = getContract();
    const tokenContract2 = getTokenContract();
    if (!contract2) return { success: false, message: "Contract not available" };
    try {
      ...
      if (tokenContract2) {
        const userAddress2 = getUserAddress();
        if (userAddress2) {
          const allowance = await tokenContract2.allowance(userAddress2, CONTRACT_ADDRESS);
          if (allowance < amountWei) {
            const approveTx = await tokenContract2.approve(CONTRACT_ADDRESS, amountWei);
            await approveTx.wait();
          }
        }
      }
      const tx = await contract2.deposit(pid, amountWei);
  ...
  async function withdraw(amount, pid = PID) {
    if (!isWalletConnected()) {
      return { success: false, message: "Please connect your wallet" };
    }
    const contract2 = getContract();
    ...
    try {
      ...
      const tx = await contract2.withdraw(pid, amountWei);
      const receipt = await tx.wait();
      return {
        success: true,
  ...
  async function claimWithdrawal() {
    ...
    if (!contract2) return { success: false, message: "Contract not available" };
    try {
      const tx = await contract2.claimWithdrawal();
      const receipt = await tx.wait();
      return {
        success: true,
  ...
  async function claim(pid = PID) {
    ...
    if (!contract2) return { success: false, message: "Contract not available" };
    try {
      const tx = await contract2.claim(pid);
      const receipt = await tx.wait();
      return {
        success: true,
